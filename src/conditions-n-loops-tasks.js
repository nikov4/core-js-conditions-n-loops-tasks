...
function convertToRomanNumerals(num) {
  const decimalNum = Math.floor(num / 10);
  let romanNum = '';
  if (decimalNum > 0) {
    for (let i = 1; i <= decimalNum; i += 1) {
      romanNum += 'X';
    }
  }
  if (num % 10 === 4) {
...
function isPalindrome(str) {
  ...
  while (str) {
    const result = iterator.next();
    if (result.done) break;
    palindrome = result.value + palindrome;
  }
  if (palindrome === str) {
    return true;
  }
...
function isContainNumber(num, digit) {
  ...
  digitString += '';
  const iterator = numString[Symbol.iterator]();
  while (numString) {
    const result = iterator.next();
    if (result.done) break;
    if (result.value === digitString) {
      return true;
    }
  }
  return false;
